...
def generateRunShell(prefix,wrapperShell,waitTime):

	count = 1
	DirName = os.path.abspath(prefix+'.runShellDir')
	shellList = []
	for i in wrapperShell:
		shellName = 'run_work.'+'%04d'%count+'.sh'
		shellPath = os.path.join(DirName,shellName)
		shellList.append(shellPath)
		with open(shellPath,'w') as fo:
			write_item = 'python runJob.py'+' '+waitTime+' '+i+'\n'
			fo.write(write_item)
		count += 1
	return shellPath
...
parser.add_argument('-wait','--wait',default = 10,type = float,help = 'set waiting time to output running time')
args = parser.parse_args()
argsDict = {"mem":args.mem,"core":args.core,"queue":args.queue,"project":args.project,"host":args.host}

runDirPrefix = os.path.splitext(args.shellPath)[0]
generateDir(runDirPrefix)
fh = open(args.shellPath,'r')
a = fh.readlines()
fh.close()
indivShellPath = generateIndivShell(runDirPrefix,a,args.line,args.flag)
runShellPath = generateRunShell(runDirPrefix,indivShellPath,args.wait)
generateSubmit(runDirPrefix,runShellPath,argsDict)
